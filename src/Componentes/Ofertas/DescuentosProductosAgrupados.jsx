...
const DescuentosProductosAgrupados = ({ onClose, tipoOferta = 5 }) => {
  ...
  const nmlDe = (ref, prod) => {
    if (!ref) return
    if (ref && ref.current) ref.innerHTML = ("")
    Product.getInstance().findByCodigoBarras({
      codigoProducto: prod.codbarra
    }, (prodres) => {
      if (prodres.length > 0) {
        const pr = prodres[0]
        if (ref && ref) {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Typography ref={(refp) => {
                              if (oferta.products &&
                                oferta.products.length > 0) {
                                nmlDe(refp, oferta.products[0])
                              }
                            }}></Typography>
                          </TableCell>
                          <TableCell align="center">
